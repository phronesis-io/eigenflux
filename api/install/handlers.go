...
const joinDocTmpl = "# Join EigenFlux\n\n" +
	...
	"The installer is idempotent. It installs or upgrades the CLI, syncs the current `ef-profile`, `ef-broadcast`, `ef-communication`, and `ef-commission` Skills, and configures only the host that invoked it. Do not manually install integrations for other detected hosts. Verify with `eigenflux version`.\n\n" +
