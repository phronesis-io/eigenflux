...
func (s *Service) SearchCommissions(ctx context.Context, req *sort.SearchCommissionsReq) (*sort.SearchCommissionsResp, error) {
	...
	var vector []float32
	if commissionID == 0 {
		embedder := embedding.NewClient(s.cfg.EmbeddingProvider, s.cfg.EmbeddingApiKey, s.cfg.EmbeddingBaseURL, s.cfg.EmbeddingModel, s.cfg.EmbeddingDimensions)
		vector, err = embedder.GetEmbedding(ctx, query)
		if err != nil {
			return commissionSearchError(fmt.Sprintf("embed Commission query: %v", err)), nil
		}
	}
	candidates, err := s.searchCommissions(ctx, query, vector, commissionID, req.GetFilters(), int(req.GetLimit()))
	if err != nil {
		return commissionSearchError(err.Error()), nil
	}
...
func (s *Service) RecommendCommissions(ctx context.Context, req *sort.RecommendCommissionsReq) (*sort.RecommendCommissionsResp, error) {
	if req.GetAgentId() <= 0 {
		return commissionRecommendError("agent_id is required"), nil
	}
	profile, err := profileDal.GetAgentProfile(s.db, req.GetAgentId())
	...
	candidates, err := s.searchCommissions(ctx, query, vector, 0, req.GetFilters(), int(req.GetLimit()))
	if err != nil {
...
func (s *Service) searchCommissions(ctx context.Context, query string, vector []float32, commissionID int64, filters *sort.CommissionSearchFilters, limit int) ([]*sort.CommissionCandidate, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 100 {
