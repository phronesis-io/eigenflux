...
func (s *Service) applyItemRerankPolicies(ctx context.Context, items []sortDal.Item, sourceMap map[int64]recallsource.Source) []sortDal.Item {
	if s.itemRerankPolicies == nil {
		return items
	}
	policies := s.itemRerankPolicies.PreRankPolicies()
...
func (s *Service) applyPostRankBoost(ctx context.Context, ranked []ranker.RankedItem, itemMap map[int64]sortDal.Item, contentClassByItem map[int64]string) []ranker.RankedItem {
	if s.itemRerankPolicies == nil {
		return ranked
	}
	policies := s.itemRerankPolicies.PostRankPolicies()
