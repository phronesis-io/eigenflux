...
// TestScoreItemsWithLRGlue exercises the pipeline glue (buildLRInput +
// scoreItemsWithLR) against a real bundle, without any infra. It drives the
// instance-owned LR manager used by the pipeline.
func TestScoreItemsWithLRGlue(t *testing.T) {
	...
	defer s.lrManager.Close()

	if !s.lrManager.Available() {
	...
	out := s.scoreItemsWithLR(ranked, items, sourceMap, profile)
	if len(out) != 2 {
