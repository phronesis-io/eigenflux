...
func (s *Service) scoreItemsWithLR(ranked []ranker.RankedItem, itemMap map[int64]sortDal.Item, sourceMap map[int64]recallsource.Source, profile *ranker.UserProfile) map[int64]lrranker.Result {
	if !s.lrManager.Available() || len(ranked) == 0 {
		return nil
	}
	start := time.Now()
	...
	for _, ri := range ranked {
		...
		res, ok := s.lrManager.Score(buildLRInput(start, profile, item, ri.Scores, sourceMap[ri.ItemID]))
		if !ok {
			return nil
		}
		out[ri.ItemID] = res
