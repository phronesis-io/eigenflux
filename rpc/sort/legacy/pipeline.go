...
func (s *Service) SortItems(ctx context.Context, req *sort.SortItemsReq) (*sort.SortItemsResp, error) {
	...
	if s.profileCache != nil {
		// Try cache first
		cachedProfile, err := s.profileCache.Get(ctx, req.AgentId)
		switch err {
		case nil:
		...
		case cache.ErrCacheMiss:
			...
			ap, _ := profileDal.GetAgentProfile(s.db, req.AgentId)
			if ap != nil && ap.Keywords != "" && ap.Status == 3 {
				...
				// Update cache
				s.profileCache.Set(ctx, &cache.CachedProfile{
					AgentID:  req.AgentId,
					Keywords: keywords,
					Domains:  domains,
					Geo:      geo,
	...
	if s.embeddingCache != nil {
		raw, err := s.embeddingCache.Get(ctx, req.AgentId)
		if err == nil && len(raw) > 0 {
			profileEmbedding = embcodec.Decode(raw)
		} else {
			// Cache miss — try DB
			ap2, _ := profileDal.GetAgentProfile(s.db, req.AgentId)
			if ap2 != nil && len(ap2.ProfileEmbedding) > 0 {
				profileEmbedding = embcodec.Decode(ap2.ProfileEmbedding)
				// Warm cache
				go s.embeddingCache.Set(context.Background(), req.AgentId, ap2.ProfileEmbedding)
			}
		}
	}
	...
	if s.rankerCfg.EnableKNNRecall && len(profileEmbedding) > 0 {
		wg.Add(1)
		go func() {
			defer wg.Done()
			filters := sortDal.BuildRecallFiltersWithExclude("", time.Now(), req.AgentId)
			knnItems, knnErr = sortDal.SearchByEmbedding(ctx, profileEmbedding, filters, s.rankerCfg.KNNRecallK, s.rankerCfg.KNNRecallCandidates)
			if knnErr != nil {
	...
	recallResults := make([]recallResult, len(s.recallSources))
	for i, src := range s.recallSources {
		wg.Add(1)
	...
	if s.searchCache != nil && len(domains) > 0 {
		...
		cacheKey = s.searchCache.BuildCacheKey(domains, keywords, geo, req.AgentId)
		logger.Ctx(ctx).Debug("search cache enabled", "key", cacheKey)

		// Use SingleFlight to deduplicate concurrent requests
		result, sfErr, _ := s.sfGroup.Do(cacheKey, func() (interface{}, error) {
			// Try cache first
			items, cacheErr := s.searchCache.Get(ctx, cacheKey)
			if cacheErr == nil {
			...
			searchReq := &sortDal.SearchItemsRequest{
				Limit:                s.cfg.KeywordRecallSize,
				Domains:              domains,
				Keywords:             keywords,
				Geo:                  geo,
				FreshnessOffset:      s.cfg.FreshnessOffset,
				FreshnessScale:       s.cfg.FreshnessScale,
				FreshnessDecay:       s.cfg.FreshnessDecay,
			...
			go func() {
				if setErr := s.searchCache.Set(context.Background(), cacheKey, cachedItems); setErr != nil {
	...
	if s.searchCache != nil && len(cachedItems) > 0 {
	...
	esItems = s.applyItemRerankPolicies(ctx, esItems, sourceMap)
	...
	contentClassByItem := s.resolveContentClasses(ctx, esItems)
	...
	allRanked := s.rankerInstance.Rank(esItems, userProfile, len(esItems))
	...
	// the served set.
	allRanked = s.applyPostRankBoost(ctx, allRanked, esItemMap, contentClassByItem)
	...
	for _, ri := range allRanked {
		...
		if ri.Score >= s.rankerCfg.MinRelevanceScore ||
	...
	logger.Ctx(ctx).Debug("relevance filter", "before", len(allRanked), "after", len(ranked), "filtered", len(filteredItems), "threshold", s.rankerCfg.MinRelevanceScore)
	...
	lrByID := s.scoreItemsWithLR(ranked, esItemMap, sourceMap, userProfile)
	if len(lrByID) > 0 {
	...
	} else if s.lrManager.Enabled() && !s.lrManager.Available() {
	...
	if s.rankerCfg.ExplorationSlots > 0 {
		explorationItems := ranker.PickExplorationItems(esItems, rankedIDs, rankedGroupIDs, s.rankerCfg.ExplorationSlots, 48*time.Hour, 0.5)
	...
	if injectRules := s.itemRerankPolicies.InjectRules(); len(injectRules) > 0 && len(candidates) > 0 {
		...
		claimed := fetchInjectClaims(ctx, s.redis, injectableIDs)
	...
	if !s.cfg.ShouldDisableDedup() && s.bf != nil {
		...
		if len(allGroupIDs) > 0 {
			seenMap, bfErr := s.bf.CheckExists(ctx, req.AgentId, allGroupIDs)
			if bfErr != nil {
	...
	} else if s.cfg.ShouldDisableDedup() {
		logger.Ctx(ctx).Info("deduplication disabled", "env", s.cfg.AppEnv)
	}
	...
	if sourceLimits := s.itemRerankPolicies.SourceLimits(); len(sourceLimits) > 0 && len(deliveryCandidates) > 0 {
	...
	claimInjectedItems(ctx, s.redis, deliveredInjected, injectClaimTTL)
	...
	} else if len(cachedItems) > 0 && len(cachedItems) >= s.cfg.KeywordRecallSize {
