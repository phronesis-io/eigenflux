...
func decode(r row) (Context, error) {
	var c Context
...
func (s Store) Get(ctx context.Context, owner, id int64, saved bool) (Context, error) {
...
func (s Store) Create(ctx context.Context, c Context, key string) (Context, error) {
	return s.CreateWithHash(ctx, c, key, c.SpecHash)
}
func (s Store) Idempotent(ctx context.Context, owner int64, key, hash string) (Context, bool, error) {
	if key == "" {
		return Context{}, false, nil
	}
	if len(key) > 128 {
		return Context{}, false, Invalid("idempotency_key", "too_long")
	}
	var r row
	...
	if r.IdempotencyHash != hash {
		return Context{}, true, Failure(409, "idempotency_conflict")
	}
	c, err := decode(r)
	return c, true, err
}
func (s Store) CreateWithHash(ctx context.Context, c Context, key, hash string) (Context, error) {
	r, e := encode(c)
	if e != nil {
		return c, e
	}
	if len(key) > 128 {
		return c, Invalid("idempotency_key", "too_long")
	}
	...
	e = s.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if c.Persistence == "saved" {
			...
			if n >= 10 {
				return Failure(409, "active_need_limit")
			}
		}
...
func (s Store) Update(ctx context.Context, c Context, expected int64) (Context, error) {
	if expected <= 0 {
		return c, Invalid("expected_revision", "required")
	}
	e := s.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if old.State == "completed" || old.State == "expired" || old.Filters.DeadlineMS != nil && *old.Filters.DeadlineMS <= c.UpdatedAt {
			return Failure(409, "terminal_need")
		}
...
func (s Store) SetState(ctx context.Context, owner, id, expected int64, state string, now int64) (Context, error) {
	var c Context
	if state != "active" && state != "paused" && state != "completed" {
		return c, Invalid("state", "invalid")
	}
	e := s.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (s Store) List(ctx context.Context, owner int64, state string, cursor int64, limit int, now int64) ([]Context, error) {
	...
	if state != "" {
		...
		default:
			return nil, Invalid("state", "invalid")
		}
	}
	if cursor > 0 {
...
func (s Store) Active(ctx context.Context, owner int64, kinds []Kind, now int64) ([]Context, error) {
	var rows []row
