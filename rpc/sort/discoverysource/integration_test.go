...
func TestPostgresESRedisThreeKinds(t *testing.T) {
	...
	mapping := map[string]any{"properties": map[string]any{"content": map[string]any{"type": "text"}, "summary": map[string]any{"type": "text"}, "embedding": map[string]any{"type": "dense_vector", "dims": 2, "index": true, "similarity": "cosine"}, "lang": map[string]any{"type": "keyword"}, "retrieval_slots": discovery.SlotsMapping()}}
	...
	source := &Source{DB: db, Redis: r, BroadcastIndex: bi, CommissionIndex: ci, AgentIndex: ai, Commission: catalogueFixture{snapshot: cat}}
	...
	engine := &discovery.Engine{Compiler: &discovery.Compiler{Taxonomy: &taxonomy.Vocabulary{Version: "fixture", Categories: []taxonomy.Node{{ID: "design", Name: "Design"}}}, Embedder: integrationEmbedding{}}, Store: discoverydb.Store{DB: db}, IDs: ids, Sources: source, Rules: rules}
	serve := discoveryserve.Service{Redis: r, IDs: ids, Executor: engine}
