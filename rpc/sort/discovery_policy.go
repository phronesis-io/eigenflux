...
func discoveryPolicies(ctx context.Context, in []discovery.Candidate, mode discovery.Mode, limit int) ([]discovery.Candidate, error) {
	...
	for _, key := range order {
		pool := buckets[key]
		if pool[0].Document.Ref.Type != discovery.Broadcast || itemRerankPolicies == nil {
			out = append(out, pool...)
			continue
		}
		...
		classes := resolveContentClasses(ctx, sourceItems)
		cs := []rank.Candidate{}
		...
		for _, p := range itemRerankPolicies.PreRankPolicies() {
			cs = p.Apply(cs)
		}
		for _, p := range itemRerankPolicies.PostRankPolicies() {
			cs = p.Apply(cs)
		}
		ttl := time.Duration(0)
		if mode == discovery.Recommendation {
			claimed := fetchInjectClaims(ctx, mq.RDB, ids)
			for _, rule := range itemRerankPolicies.InjectRules() {
				rule := rule
		...
		for _, rule := range itemRerankPolicies.SourceLimits() {
			rule := rule
