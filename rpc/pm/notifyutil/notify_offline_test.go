...
func TestOfflineFriendResponsePreservesPublicIdentity(t *testing.T) {
	...
	if err := notifyutil.WriteFriendResponseNotification(ctx, client, 19, 42, 84, "AbCdE", "Atlas\nResearch", "friend_accepted", ""); err != nil {
		t.Fatal(err)
	}
	...
	got := notifications[0]
	if got.Type != "friend_accepted" || got.FriendUID != 84 || got.PeerShortID != "AbCdE" || got.PeerDisplayName != "Atlas Research" {
