...
// TestUpdateService_PartialUpdate verifies that a partial update only writes the
// fields the caller passed, leaving the rest untouched. Regression test for a
// handler bug that wrote every Thrift field into the GORM updates map — an
// untouched call_spec_schema arrived as "" and crashed the jsonb column.
func TestUpdateService_PartialUpdate(t *testing.T) {
	cli := newTradeClient(t)
	sellerID := createTestAgentID()
	t.Cleanup(func() { cleanTradeData(t, sellerID) })

	serviceID := publishTestService(t, cli, sellerID, "PartialUpdate")
	...
	require.NoError(t, err)
	require.Equal(t, int32(0), updateResp.BaseResp.Code, "partial UpdateService failed: %s", updateResp.BaseResp.Msg)
	...
	require.NotNil(t, got, "updated service should appear in GetMyServices")
	assert.Equal(t, "Partial Update Title", got.Title, "title should be updated")
	assert.Equal(t, "10 USDC", got.PriceText, "price_text should be untouched")
	assert.Equal(t, int64(10_000_000), got.AmountAtomic, "amount_atomic should be untouched")
	assert.Equal(t, int64(3_600_000), got.DeliveryDeadlineMs, "delivery_deadline_ms should be untouched")
	assert.Equal(t, "USDC", got.Asset, "asset should be untouched")
	assert.Equal(t, "A capability for testing purposes", got.CapabilityDesc, "capability_desc should be untouched")
}
