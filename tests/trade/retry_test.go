...
func TestRefundOrder_RetryIsIdempotent(t *testing.T) {
	cli := newTradeClient(t)
	sellerID := createTestAgentID()
	buyerID := createTestAgentID()
	defer cleanTradeData(t, sellerID, buyerID)

	svcID := publishTestService(t, cli, sellerID, "retry-refund")

	createResp, err := cli.CreateOrder(context.Background(), &trade.CreateOrderReq{
		BuyerAgentId:   buyerID,
		ServiceId:      svcID,
		BuyerInput:     "x",
		IdempotencyKey: fmt.Sprintf("retry-refund-%d", time.Now().UnixNano()),
	})
	require.NoError(t, err)
	orderID := createResp.OrderId
	...
	r1, err := cli.RefundOrder(context.Background(), &trade.RefundOrderReq{
		OrderId: orderID, ActorAgentId: buyerID,
	})
	require.NoError(t, err)
	require.Equal(t, int32(0), r1.BaseResp.Code, "msg=%s", r1.BaseResp.Msg)

	r2, err := cli.RefundOrder(context.Background(), &trade.RefundOrderReq{
		OrderId: orderID, ActorAgentId: buyerID,
	})
	require.NoError(t, err)
	require.Equal(t, int32(0), r2.BaseResp.Code, "retry must return success, got msg=%s", r2.BaseResp.Msg)

	var eventCount int64
	require.NoError(t, testutil.TestDB.QueryRow(
		"SELECT COUNT(*) FROM trade_order_events WHERE order_id = $1 AND event_type = $2",
		orderID, int16(5), // EventTypeRefunded
	).Scan(&eventCount))
	require.Equal(t, int64(1), eventCount, "exactly one refunded event row")
	...
	require.Equal(t, int64(1), receiptCount, "exactly one refunded receipt row")

	orderFilter := fmt.Sprintf(`{"order_id": "%d", "event_type": "refunded"}`, orderID)
	var outboxCount int64
	require.NoError(t, testutil.TestDB.QueryRow(
		"SELECT COUNT(*) FROM trade_outbox WHERE payload_json @> $1::jsonb",
		orderFilter,
	).Scan(&outboxCount))
	require.Equal(t, int64(1), outboxCount, "exactly one refunded outbox row")
}
