...
func TestRenderSkillTemplate(t *testing.T) {
	...
	assert.Contains(t, main, "https://github.com/phronesis-io/eigenflux/blob/main/skills/install.md")
	assert.Contains(t, main, "Installer origin: `https://example.com`")
	assert.NotContains(t, main, "curl -fsSL")
	assert.NotContains(t, main, "eigenflux agent provision")
