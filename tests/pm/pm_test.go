...
func TestFetchPM_RESTIncludesHistory(t *testing.T) {
	testutil.WaitForAPI(t)
	emails := []string{"pm_rest_author@test.com", "pm_rest_user@test.com"}
	testutil.CleanupTestEmails(t, emails...)

	author := testutil.RegisterAgent(t, "pm_rest_author@test.com", "RA", "bio")
	user := testutil.RegisterAgent(t, "pm_rest_user@test.com", "RU", "bio")
	authorID, _ := strconv.ParseInt(author["agent_id"].(string), 10, 64)
	userID, _ := strconv.ParseInt(user["agent_id"].(string), 10, 64)

	itemID := int64(7780200)
	mockItem(t, itemID, authorID, "")
	defer cleanMockItems(t, itemID)
	defer cleanPMData(t, authorID, userID)
	...
	// user→author (A)
	r1 := testutil.DoPost(t, "/api/v1/pm/send", map[string]string{
		"content": "A", "item_id": strconv.FormatInt(itemID, 10),
	}, user["token"].(string))
	if int(r1["code"].(float64)) != 0 {
		t.Fatalf("send A failed: %v", r1["msg"])
	}
	// First fetch — marks A as read; extract conv_id from returned message A.
	fetchResp1 := testutil.DoGet(t, "/api/v1/pm/fetch", author["token"].(string))
	if int(fetchResp1["code"].(float64)) != 0 {
		t.Fatalf("first fetch failed: %v", fetchResp1["msg"])
	}
	convID := convIDFromFetchResp(t, fetchResp1)

	// author→user (B)
	r2 := testutil.DoPost(t, "/api/v1/pm/send", map[string]string{
		"content": "B", "conv_id": convID,
	}, author["token"].(string))
	if int(r2["code"].(float64)) != 0 {
		t.Fatalf("send B failed: %v", r2["msg"])
	}
	// user→author (C, unread for author)
	r3 := testutil.DoPost(t, "/api/v1/pm/send", map[string]string{
		"content": "C", "conv_id": convID,
	}, user["token"].(string))
	if int(r3["code"].(float64)) != 0 {
		t.Fatalf("send C failed: %v", r3["msg"])
	}

	// Second fetch — expect A,B in history; C in messages; no overlap.
	resp := testutil.DoGet(t, "/api/v1/pm/fetch", author["token"].(string))
	if int(resp["code"].(float64)) != 0 {
		t.Fatalf("fetch failed: %v", resp["msg"])
	}
	...
	rawHist, _ := data["history_messages"].([]interface{})
	rawMsgs, _ := data["messages"].([]interface{})

	histContents := contentSet(rawHist)
	msgsContents := contentSet(rawMsgs)

	if !histContents["A"] || !histContents["B"] {
		t.Errorf("history missing A and/or B: %v", histContents)
	}
	if !msgsContents["C"] {
		t.Errorf("messages missing C: %v", msgsContents)
	}
	for k := range histContents {
		if msgsContents[k] {
			t.Errorf("content %q appears in both history and messages", k)
		}
	}
}

func contentSet(list []interface{}) map[string]bool {
