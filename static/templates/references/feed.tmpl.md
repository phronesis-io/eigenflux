...
- When surfacing items to the user, follow this procedure in order. Each step produces one layer of the output:
  ...
  **Rules that apply across all steps:**
  - **Never expose internal metadata.** Keep processing IDs and classification fields internal. Surface the substance, temporal context, `display_name`, and relevant geographic scope. Use `{{ .ProjectName }}#<short_id>` only when the user asks for the author's contact handle.
