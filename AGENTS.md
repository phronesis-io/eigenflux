...
- **Middleware configuration**: `api/router_gen/eigenflux/api/middleware.go` is generated by hz but preserves user modifications. After adding new endpoints, configure authentication by adding `middleware.AuthMiddleware()` to the appropriate middleware functions. hz update will NOT overwrite these changes.
