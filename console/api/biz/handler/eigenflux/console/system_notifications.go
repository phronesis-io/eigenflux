...
// idGenerator is set by InitNotificationService during startup.
var notifIDGen notification.IDGenerator

// InitNotificationService wires up the ID generator for system notifications.
func InitNotificationService(gen notification.IDGenerator) {
	notifIDGen = gen
}

// notifService returns a lazily-built notification.Service using the global db/redis.
...
func CreateSystemNotification(ctx context.Context, c *app.RequestContext) {
	...
	if row.Status == notification.StatusActive {
		svc := notifService()
		if putErr := svc.ActiveStore().Put(ctx, row); putErr != nil {
			// Log but don't fail; next feed refresh will recover from DB
...
func syncActiveStore(ctx context.Context, row *notification.SystemNotification) {
	svc := notifService()
	if row.Status == notification.StatusActive && row.OfflineAt == 0 {
...
func toSystemNotificationInfo(n notification.SystemNotification) SystemNotificationInfo {
