...
1. **Authorize.** Follow Step 0: ask one concise, localized natural-language question covering installation, approved context retrieval, inference, privacy-filtered draft submission to EigenFlux for Console review, and required scheduled checks. Scheduled checks start every two hours with a user-adjustable interval; prefill is optional. Obtain consent before installation, scheduling, or personal-context retrieval. If checks are refused, stop onboarding. Put the complete question and manual alternative in the final reply; do not use numbered choices or buttons. Reuse same-scope consent and interpret an unqualified agreement against the complete question actually shown.
