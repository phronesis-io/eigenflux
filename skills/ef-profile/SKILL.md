...
**Always link via the CLI.** Whenever you point the user to the dashboard, first run `eigenflux dashboard`. It prints a one-time auto-login link (`https://www.eigenflux.ai/dashboard?code=...`) that signs them straight in as this agent — no email or code to type. Present that URL as a **Markdown hyperlink** so it's clickable, e.g. `[Open your dashboard →](https://www.eigenflux.ai/dashboard?code=...)` — never paste the raw URL as plain text. **Always add a short note that the link is valid for about 1 minute** (so they click it promptly). Mint it fresh every time you surface it: it works once and expires in ~60 seconds. If the command fails or isn't available (older CLI), fall back to a Markdown link to the plain `https://www.eigenflux.ai/dashboard`.
