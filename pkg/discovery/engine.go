...
func (e *Engine) Execute(ctx context.Context, owner int64, r Request, mode Mode, now int64) (x Execution, resultErr error) {
	...
	// Keep a reserve so a source withdrawn during execution cannot prevent an
	// already eligible successor from being served.
	ranked := Merge(candidates, r.SourceKinds, mode, r.Limit*3)
	refs := []Document{}
	for _, c := range ranked {
		refs = append(refs, c.Document)
	}
	fresh, err := e.Sources.Hydrate(ctx, owner, mode, refs)
	if err != nil {
		return x, err
	}
	...
	for _, candidate := range ranked {
		c := candidate.Context
		if !verified[c.ID] {
			current, err := e.Store.Get(ctx, owner, c.ID, false)
			if err != nil {
				return x, err
			}
			if !current.Active(now) || current.Revision != c.Revision {
				return x, Failure(409, "context_changed")
			}
			verified[c.ID] = true
		}
		d, ok := byRef[candidate.Document.Ref.Key()]
		if !ok || d.Version != candidate.Document.Version || Check(c, d, mode, now) != "" {
			continue
		}
		x.Candidates = append(x.Candidates, candidate)
		if len(x.Candidates) >= r.Limit {
			break
		}
	}
	x.Status = "ok"
...
// Revalidate runs again at the serving boundary and on idempotent response
// replay. It uses the current clock/authority, never stale cache eligibility.
func (e *Engine) Revalidate(ctx context.Context, owner int64, x Execution, now int64) error {
	verified := map[int64]bool{}
	var ownerInfo *OwnerContext
	for _, c := range x.Contexts {
		if c.OwnerID != owner {
			return Failure(404, "context_not_found")
		}
		current, err := e.Store.Get(ctx, owner, c.ID, false)
		if err != nil {
			return err
		}
		if !current.Active(now) || current.Revision != c.Revision || current.SpecHash != c.SpecHash {
			return Failure(409, "stale_result")
		}
		if c.Origin == "agent_context" || c.Origin == "baseline" {
			if ownerInfo == nil {
				v, err := e.Sources.Owner(ctx, owner)
				if err != nil {
					return err
				}
				ownerInfo = &v
			}
			if ownerInfo.Revision != c.SourceRevision {
				return Failure(409, "stale_result")
			}
		}
		if c.SourceNeedID > 0 {
			parent, err := e.Store.Get(ctx, owner, c.SourceNeedID, true)
			if err != nil {
				return err
			}
			if !parent.Active(now) || parent.Revision != c.SourceNeedRevision {
				return Failure(409, "stale_result")
			}
		}
		verified[c.ID] = true
	...
	for _, c := range x.Candidates {
		if !verified[c.Context.ID] {
			return Failure(409, "stale_result")
		}
		docs = append(docs, c.Document)
	}
	fresh, err := e.Sources.Hydrate(ctx, owner, x.Mode, docs)
	if err != nil {
		return err
	}
	byRef := map[string]Document{}
	...
	for _, c := range x.Candidates {
		d, ok := byRef[c.Document.Ref.Key()]
		if !ok || d.Version != c.Document.Version || Check(c.Context, d, x.Mode, now) != "" {
			return Failure(409, "stale_result")
		}
	}
	return nil
...
// RefreshPage validates the frozen contexts once and filters a bounded legacy
// reserve against current authority and automatic history. It never reranks a
// new corpus under an existing impression.
func (e *Engine) RefreshPage(ctx context.Context, owner int64, x Execution, now int64) (Execution, error) {
	if x.Mode != Recommendation || len(x.Candidates) > 20 {
		return x, Invalid("page", "invalid")
	}
	metadata := x
	metadata.Candidates = nil
	if err := e.Revalidate(ctx, owner, metadata, now); err != nil {
		return x, err
	}
	docs := make([]Document, 0, len(x.Candidates))
	for _, c := range x.Candidates {
		docs = append(docs, c.Document)
	}
	fresh, err := e.Sources.Hydrate(ctx, owner, Recommendation, docs)
	if err != nil {
		return x, err
	}
	seen, err := e.Sources.Seen(ctx, owner, fresh)
	...
	for _, c := range x.Candidates {
		d, ok := byRef[c.Document.Ref.Key()]
		if !ok || d.Version != c.Document.Version || seen[d.Ref.Key()] || Check(c.Context, d, Recommendation, now) != "" {
			continue
		}
		current := ScoreRules(c.Context, d, e.Rules[d.Ref.Type][Recommendation], now)
		if current.ConfigHash != c.Score.ConfigHash {
			return x, Failure(409, "stale_result")
		}
		out = append(out, c)
	}
	x.Candidates = out
