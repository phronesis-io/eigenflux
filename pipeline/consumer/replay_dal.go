...
func batchInsertReplayLogs(tx *gorm.DB, logs []ReplayLog) error {
	...
	for i := 0; i < len(logs); i += batchSize {
		...
		cols := "(id, request_id, agent_id, item_id, agent_features, item_features, item_score, position, served_at, created_at)"
		placeholders := make([]string, 0, len(batch))
		args := make([]interface{}, 0, len(batch)*10)
		for _, l := range batch {
			placeholders = append(placeholders, "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
			args = append(args, l.ID, l.RequestID, l.AgentID, l.ItemID,
				l.AgentFeatures, l.ItemFeatures, l.ItemScore, l.Position,
				l.ServedAt, l.CreatedAt)
		}

		sql := fmt.Sprintf("INSERT INTO replay_logs %s VALUES %s", cols, strings.Join(placeholders, ", "))
