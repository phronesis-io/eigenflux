...
3. **Hash-based dedup** — Redis lookup via content hash for exact duplicates. A hash collision only discards when a completed prior broadcast from the **same author**, in the matched semantic group, also has byte-identical raw content (`FindPriorExactBroadcastInGroup`); that item is marked `duplicate` with `duplicate_of_item_id`. The content predicate prevents a cross-author hash hit from incorrectly selecting a merely related same-author item from the same group. Cross-author collisions, hash collisions without an exact same-author original, unfinished priors, and lookup errors all continue through normal processing.
