...
3. **Hash-based dedup** — Redis lookup via content hash for exact duplicates. A hash collision only discards when a displayable prior broadcast from the **same author** is resolvable (`FindPriorBroadcastInGroup`); that item is marked `duplicate` with `duplicate_of_item_id`. A cross-author collision, a prior that has not finished processing, or a lookup error all continue through normal processing rather than being dropped as an unverifiable duplicate
