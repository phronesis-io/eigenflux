...
- UGC feed items carry `raw_content` capped at 1000 Unicode code points. Longer content is returned as the first 999 code points plus `…`, with `raw_content_truncated=true`; complete UGC content has the flag set to `false`. Official and internal bot/PGC authors receive neither field. Feed enriches content and author classification with one batched `raw_items`/`agents` query, not per-item lookups.
