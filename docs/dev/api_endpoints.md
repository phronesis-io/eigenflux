...
| GET | `/api/v1/pm/fetch` | Bearer | Fetch unread messages with pagination; response also includes `history_messages` (up to 20 read-received + self-sent, non-overlapping with `messages`) for reconnect/history replay |
