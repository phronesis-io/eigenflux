...
Serving calls accept owner-scoped `Idempotency-Key`: same key/body returns the same IDs for 24 hours; different body conflicts. Replay the assembled response without rechecking Need/source state; new requests observe current state. Capture uses the existing `/api/v2/need-inputs` API and durable idempotency; serving idempotency is Redis-backed and errors retriably if that guarantee cannot be maintained.
...
Add methods/types through existing IDLs and code generation; preserve legacy field/method IDs. Need capture remains in the gateway and `pkg/need`; taxonomy enters Sort. New serving methods enter Feed, which calls context-based Sort execution; legacy methods act as adapters, not parallel new ranking implementations.
...
Starting per-context broadcast quotas: lexical 80, dense 80, structured 40, hot 20, new 20, UGC 10; union cap 200. Commission/Agent each use lexical/dense/structured with cap 100. Explicit text queries with reviewed alias variants additionally run a synonym channel capped at 40 per kind; existing union and concurrency bounds stay in force. Deduplicate per context/kind; merge channel ranks by fixed quotas/round-robin, not incomparable raw scores. Query mode disables hot/new/UGC and exposure-injection filler. An automatic request evaluates up to five contexts, with at most three contexts and six backend calls concurrently; hard global cap 1,000 pairs. Multi-kind contexts divide that cap before execution. Legacy limit-100 commission search has a bounded exception pool of up to 300 for hydration; it never creates unbounded fan-out.

Query merging: prioritize eligible exact identity matches within each kind, preserving stable tie order and exact provenance during typed-source deduplication. Select candidates round-robin in explicit client kind order or default `broadcast, commission, agent`, skipping exhausted kinds and stopping at the combined limit. Then group each selected page into contiguous type blocks, with exact matches first inside their own block. Keep the existing `items` array. Grouping must not move candidates between pages: freeze the final page order before delivery so cursor retries and replay positions match the response. Recommendation selection retains Need priority and total limit, then groups the selected results by type. This is deterministic coverage, not a global relevance comparison.
...
| Query synonym expansion | Existing ES text fields and reviewed taxonomy aliases | Bounded query-time variants, lower lexical boost, target-phrase requirement and the same hard filters; no mapping change |
...
Reuse existing discoverability/Card access rules rather than create a new contact permission system. Recheck viewer-specific self/block/relationship exclusions through the authoritative domain. `agent` recommendations exclude existing friends or active conversations for new-contact discovery; literal query search may rediscover already-known people but never blocked/inaccessible/self results. These are mode-specific eligibility rules recorded with the plan. Existing relationship queries need a bounded batch adapter if they are currently per-ID.
...
| Mapped Need IDs | Soft relevance | Soft relevance | Soft relevance |
...
Every new execution reads eligibility once and hydrates candidates once. In-flight
execution, assembled response retries and pages do not revalidate mutable Need
state. Existing account deletion/reset includes execution contexts and caches.
